...
fn view_attributes(state: &CharacterSheet) -> Element<'_, Message> {
    let attr_row = |label: &'static str, val: i32| {
        row![
            ...
            text(val.to_string()).size(20).width(30).align_x(alignment::Horizontal::Center),
        ]
        .spacing(10)
        .align_y(Alignment::Center)
    };
...
fn view_vitals(state: &CharacterSheet) -> Element<'_, Message> {
    ...
    column![
        ...
        resource_ticker("Spells:", &state.spells_input, spell_slots_max, Message::SpellsInputChanged, Message::AdjustSpells),
        resource_ticker("Miracles:", &state.miracles_input, miracle_slots_max, Message::MiraclesInputChanged, Message::AdjustMiracles),
    ]
    .spacing(10)
    .padding(10)
    .into()
...
fn view_editor(state: &CharacterSheet) -> Element<'_, Message> {
    ...
    let content = column![
        ...
        row![
            text("Level:"),
            text_input("1", &state.level_input).on_input(Message::LevelChanged).width(50)
        ].spacing(10).align_y(Alignment::Center),
        row![
            text("Origin:"),
            pick_list(Origin::all(), Some(state.character.origin), Message::OriginSelected)
        ].spacing(10).align_y(Alignment::Center),
        column![
            text("Attributes").size(20),
            attr_row("Strength", state.character.attributes.strength, AttributeField::Strength),
            attr_row("Dexterity", state.character.attributes.dexterity, AttributeField::Dexterity),
            attr_row("Endurance", state.character.attributes.endurance, AttributeField::Endurance),
            attr_row("Faith", state.character.attributes.faith, AttributeField::Faith),
            attr_row("Will", state.character.attributes.will, AttributeField::Will),
            attr_row("Intelligence", state.character.attributes.intelligence, AttributeField::Intelligence),
            attr_row("Luck", state.character.attributes.luck, AttributeField::Luck),
        ].spacing(10),
...
fn view_inventory(state: &CharacterSheet) -> Element<'_, Message> {
    ...
    column![
        text(format!("Inventory ({}/{})", state.character.inventory.iter().filter(|s| !s.is_empty()).count(), total_slots)).size(24),
        slots_col
